import
  std/json,
  kaz/[broadcast, global, replay_runtime, replays, sim]
...
## used to die with kaz_error_len() == 0 and no diagnostic at all). The
...
proc kazLoadReplay(data: ptr uint8, length: cint): cint
    {.exportc: "kaz_load_replay", cdecl.} =
  try:
    lastError = ""
...
proc kazInput(data: ptr uint8, length: cint)
    {.exportc: "kaz_input", cdecl.} =
  if runtimeLoaded:
    viewer.applyGlobalViewerMessage(data.bytesFromPointer(int(length)))

proc kazFrame(): cint {.exportc: "kaz_frame", cdecl.} =
...
proc kazPacketPointer(): ptr uint8
    {.exportc: "kaz_packet_ptr", cdecl.} =
  if packet.len == 0:
    nil
  else:
    packet[0].addr

proc kazPacketLength(): cint {.exportc: "kaz_packet_len", cdecl.} =
  cint(packet.len)

proc kazMismatchTick(): cint {.exportc: "kaz_mismatch_tick", cdecl.} =
  if runtimeLoaded:
    cint(replay.hashMismatchTick)
  else:
    -1

proc kazErrorPointer(): ptr uint8 {.exportc: "kaz_error_ptr", cdecl.} =
  if lastError.len == 0:
    nil
  else:
    cast[ptr uint8](lastError[0].addr)

proc kazErrorLength(): cint {.exportc: "kaz_error_len", cdecl.} =
  cint(lastError.len)

proc kazStagePointer(): ptr uint8 {.exportc: "kaz_stage_ptr", cdecl.} =
  ## The progress note (see stageNote above). Unlike kaz_error_*, this stays
...
proc kazStageLength(): cint {.exportc: "kaz_stage_len", cdecl.} =
  cint(stageNoteLen)
...
when isMainModule and defined(emscripten):
  ...
  # module stays alive and JS keeps calling kaz_load_replay/kaz_frame. The
