    ...
    function onWorkerMessage(event) {
      ...
      try {
        if (message.type === 'text') {
          ...
          // The `coworld-replay` bridge's `ready` is posted HERE — from the
          // branch that has just set data-replay-loaded, which the Worker only
          // sends after ingestPacket() handed BroadcastCore the first frame
          // and it drew — never on rAF timing at the call site. Posting it
          // early makes the softmax.com embed sample an unpainted shell: the
          // clock reads the placeholder and the scrubber is dead, while every
          // gate downstream is green (chorus 3c11c953, 2026-08-24).
