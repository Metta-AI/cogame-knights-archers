...
##
## A seat that sets neither is `phalanx`. To field your own policy, reuse
## this image and set PLAYER_PROMPT:
##
##   coworld upload-policy coworld-knights-archers:latest --name my-warden \
...
proc readyBlob(): string =
  ...
  ## advance the tick as soon as all four seats have acknowledged the frame.
  result = newString(1)
  result[0] = char(0x85)
...
when isMainModule:
  ...
  proc dial(attempts: int): WebSocket =
    ...
    for attempt in 0 ..< attempts:
      try:
        ...
        if attempt == 0:
          echo "knights-archers player: game not listening yet (", error.msg,
            "); retrying"
  ...
  if socket == nil:
    quit("knights-archers player: game never accepted a connection", 1)
  echo "knights-archers player: connected"
  ...
  # episode (the paintball round-3 scar, 2026-08-25). The server now holds an
  ...
  while true:
    ...
    if sessionFrames == 0 or reconnects >= ReconnectAttempts:
      break
    inc reconnects
    echo "knights-archers player: re-dialling the seat (attempt ", reconnects, ")"
    socket = dial(ReconnectAttempts)
    if socket == nil:
      echo "knights-archers player: game is no longer listening, exiting cleanly"
      break
    echo "knights-archers player: reconnected, re-registering"
  quit(0)
