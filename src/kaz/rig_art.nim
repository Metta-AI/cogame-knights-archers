...
proc gameDir*(): string =
  ## Returns the knights-archers game directory.
  getCurrentDir()
...
proc loadCrewSpriteRow*(row: int, label: string): seq[CrewSprite] =
  ...
  if row < 0:
    raise newException(KazError, "Crew sprite sheet row is negative.")
  let
    path = crewSheetPath()
...
## centerline at image mid-height) mounts MaceGripPx east of the body center
...
proc ensureMaceLoaded() =
  ## The KNIGHT's mace, COMPOSITED at startup rather than read off disk: a
  ## 30 px haft with a 12 px spiked head, drawn head EAST on the image
  ## mid-line, which is the same convention the rig's grip math already
  ## mounts. Procedural because the whole point of the two silhouettes is
  ## that a spectator reads a knight from an archer at board scale WITHOUT a
  ## label, and a hand-drawn 12 px head cannot be authored more precisely
  ## than this.
  if maceLoaded:
    return
  const
    W = 168        ## master canvas; scaled down to MaceLengthPx by width.
    H = 96
  var image = newImage(W, H)
  let mid = float32(H) / 2
  # The haft: a tapered dark shaft from the grip (west) to the head (east).
  var haft = newPath()
  haft.rect(rect(vec2(6, mid - 5), vec2(112, 10)))
  image.fillPath(haft, rgba(84, 60, 34, 255).color)
  var collar = newPath()
  collar.rect(rect(vec2(104, mid - 9), vec2(12, 18)))
  image.fillPath(collar, rgba(150, 150, 158, 255).color)
  # The head: a steel disc with six spikes around it.
  var head = newPath()
  head.circle(circle(vec2(132, mid), 24))
  image.fillPath(head, rgba(196, 198, 206, 255).color)
  for i in 0 ..< 6:
    let
      a = float32(i) * 2.0'f32 * PI.float32 / 6.0'f32
      cx = 132'f32 + cos(a) * 22'f32
      cy = mid + sin(a) * 22'f32
    var spike = newPath()
    spike.circle(circle(vec2(cx, cy), 9))
    image.fillPath(spike, rgba(226, 228, 234, 255).color)
  var core = newPath()
  core.circle(circle(vec2(132, mid), 12))
  image.fillPath(core, rgba(120, 122, 132, 255).color)
  maceMaster = image
  maceScale = float(MaceLengthPx) / max(1.0, float(maceMaster.width))
  maceLoaded = true

proc ensureBowLoaded() =
  ## The ARCHER's bow: a 26 px arc with a string, same head-EAST convention so
  ## the identical grip math mounts it and an arrow leaves the string exactly
  ## where the mace's head swings. A short, open, C-shaped silhouette against
  ## the mace's long solid one — that difference is how a viewer tells the two
  ## roles apart with the HUD off.
  if bowLoaded:
    return
  const
    W = 132
  ...
  var image = newImage(W, H)
  let mid = float32(H) / 2
  # The limb: a thick open arc opening WEST (the bow is held with the belly
  # toward the archer and the string behind it).
  var limb = newPath()
  limb.arc(52'f32, mid, 52'f32, -1.15'f32, 1.15'f32, false)
  image.strokePath(limb, rgba(126, 88, 44, 255).color, strokeWidth = 11)
  # The string: a straight line between the two tips.
  var str = newPath()
  str.moveTo(vec2(52'f32 + cos(-1.15'f32) * 52'f32,
                  mid + sin(-1.15'f32) * 52'f32))
  str.lineTo(vec2(52'f32 + cos(1.15'f32) * 52'f32,
                  mid + sin(1.15'f32) * 52'f32))
  image.strokePath(str, rgba(230, 226, 210, 235).color, strokeWidth = 4)
  # The nocked arrow, pointing EAST along the aim.
  var shaft = newPath()
  shaft.rect(rect(vec2(48, mid - 2), vec2(66, 4)))
  image.fillPath(shaft, rgba(214, 206, 186, 255).color)
  var tip = newPath()
  tip.moveTo(vec2(114, mid - 6))
  tip.lineTo(vec2(126, mid))
  tip.lineTo(vec2(114, mid + 6))
  tip.closePath()
  image.fillPath(tip, rgba(206, 210, 220, 255).color)
  bowMaster = image
  bowScale = float(BowLengthPx) / max(1.0, float(bowMaster.width))
  bowLoaded = true
...
proc soldierRotPixels*(
  ...
  let
    ...
    # MaceGripPx along the aim (stock behind the hub, barrel reaching out front)
    # and WeaponRightPx off the aim ray to the cog's RIGHT (+y = right when facing
    # +x/east); it spins with the unit so the marker rides the head's right.
    gunMat =
      unitRot *
      translate(vec2(
        float32(MaceGripPx * renderScale), float32(WeaponRightPx * renderScale))) *
      scale(vec2(
        float32(maceScale * float(renderScale)),
        float32(maceScale * float(renderScale))
      )) *
      translate(vec2(0, float32(-maceMaster.height) / 2))
  canvas.draw(master, bodyMat)
  canvas.draw(maceMaster, gunMat)
...
proc rigSegPixels*(team: Team, seg: RigSeg, baseStep, artStep: int,
  ...
  # (rigMacePixels), drawn ABOVE the head with a backlight glow so it reads clearly
...
var rigMaceCache: array[Team, seq[tuple[aimStep, scale: int, pixels: seq[uint8]]]]
...
proc rigHeldWeaponPixels(
  ...
  ## (WeaponRightPx off the aim ray, gripPx along aim) with its business end on
  ...
  let
    ...
    mat =
      ...
      translate(vec2(
        float32(gripPx * renderScale), float32(WeaponRightPx * renderScale))) *
  ...
  let glow = weaponLayer.shadow(
    offset = vec2(0, 0),
    spread = float32(WeaponGlowSpread * float(renderScale)),
    blur = float32(WeaponGlowRadius * float(renderScale)),
    color = rgba(255, 214, 138, WeaponGlowAlpha).color)  # faint warm rim light
  var canvas = newImage(outCanvas, outCanvas)
...
proc rigMacePixels*(team: Team, aimStep: int, renderScale = 1): seq[uint8] =
  ## The held MACE as its OWN HUB-centered rig object (not baked into the head): mounted at the cog's RIGHT (WeaponRightPx off the aim ray,
  ## stock MaceGripPx along aim), barrel on +aim so tracers line up. A soft warm
  ...
  ensureMaceLoaded()
  rigHeldWeaponPixels(
    rigMaceCache, team, aimStep, renderScale, maceMaster, maceScale, MaceGripPx)

var rigBowCache: array[Team, seq[tuple[aimStep, scale: int, pixels: seq[uint8]]]]

proc rigBowPixels*(team: Team, aimStep: int, renderScale = 1): seq[uint8] =
  ## The held BOW, the swap-in for rigMacePixels on an ARCHER seat:
  ## same grip (the cog's RIGHT, WeaponRightPx off the aim ray) and the same
  ## nozzle-on-+aim convention, so the spray cone leaves the nozzle exactly where
  ## tracers leave the muzzle. Mounted BowGripPx along aim and scaled to
  ## BowLengthPx: a can is a short fistful, so its silhouette reads clearly
  ## different from the long marker — that difference is how a viewer tells which
  ## weapon a cog is holding.
  ensureBowLoaded()
  rigHeldWeaponPixels(
    rigBowCache, team, aimStep, renderScale, bowMaster, bowScale,
    BowGripPx)
...
# ---------------------------------------------------------------------------
#  KNIGHTS-ARCHERS: the horde and the arrows.
#
#  Both are composited at startup from art that is already in the repo — no
#  placeholders, no downloads. A zombie is the shipped GREEN cog family run
#  through a desaturating, mottled palette pass and a two-frame shamble, so it
#  reads as a CORRUPTED cog rather than as a third team; the arrow is drawn
#  procedurally by the same pixie compositor that bakes the mace and the bow.
# ---------------------------------------------------------------------------

const
  ZombieShambleFrames* = 2      ## the shamble cycle: a corrupted cog limps.
  ArrowRotations* = SoldierRotations
  ArrowCanvas* = 24             ## px square canvas for one arrow in flight.
  ArrowShaftPx* = 16            ## px shaft length.
  ArrowFletchPx* = 4            ## px fletch length at the tail.

var
  zombieRotCache: array[
    ZombieShambleFrames,
    array[SoldierRotations, seq[tuple[scale: int, pixels: seq[uint8]]]]]
  arrowRotCache: array[ArrowRotations, seq[tuple[scale: int, pixels: seq[uint8]]]]

proc zombieRotPixels*(
  rot: int, shamble: int, renderScale = 1
): seq[uint8] =
  ## One pre-rendered zombie sprite: the shipped green cog, desaturated toward
  ## grave-grey, mottled by a hash of the pixel so no two patches rot the same
  ## way, and limped one shamble frame. Cached per (frame, rotation, scale) the
  ## same way the soldier cache is, so a forty-strong horde defines at most
  ## `ZombieShambleFrames x SoldierRotations` sprites for the whole episode.
  let
    r = ((rot mod SoldierRotations) + SoldierRotations) mod SoldierRotations
    f = ((shamble mod ZombieShambleFrames) + ZombieShambleFrames) mod
      ZombieShambleFrames
  for cached in zombieRotCache[f][r]:
    if cached.scale == renderScale:
      return cached.pixels
  ## The GREEN master is the source, so the silhouette is a real cog. Only the
  ## palette and one row-shear change.
  var pixels = soldierRotPixels(Green, DefaultSkin, r, renderScale)
  let side = SoldierCanvas * renderScale
  var rotted = newSeq[uint8](pixels.len)
  for y in 0 ..< side:
    # The shamble: frame 1 shears every other row one pixel, which reads as a
    # limp at board scale without a second art master.
    let shift =
      if f == 1 and (y mod 4) < 2: 1 else: 0
    for x in 0 ..< side:
      let
        src = (y * side + max(0, x - shift)) * 4
        dst = (y * side + x) * 4
      if pixels[src + 3] == 0'u8:
        continue
      let
        r0 = int(pixels[src])
        g0 = int(pixels[src + 1])
        b0 = int(pixels[src + 2])
        luma = (r0 * 30 + g0 * 59 + b0 * 11) div 100
      ...
      let
        ...
        rr = clamp((r0 + luma * 2) div 3 * 78 div 100 + mottle, 0, 255)
        gg = clamp((g0 + luma * 2) div 3 * 96 div 100 + mottle, 0, 255)
        bb = clamp((b0 + luma * 2) div 3 * 62 div 100 + mottle, 0, 255)
      rotted[dst] = uint8(rr)
      rotted[dst + 1] = uint8(gg)
      rotted[dst + 2] = uint8(bb)
      rotted[dst + 3] = pixels[src + 3]
  zombieRotCache[f][r].add((scale: renderScale, pixels: rotted))
  rotted

proc arrowRotPixels*(rot: int, renderScale = 1): seq[uint8] =
  ## One arrow in flight: a 16 px shaft with a 4 px fletch and a steel head,
  ## drawn at the aim step it is travelling on. Procedural, by the same pixie
  ## compositor that bakes the mace and the bow.
  let r = ((rot mod ArrowRotations) + ArrowRotations) mod ArrowRotations
  for cached in arrowRotCache[r]:
    if cached.scale == renderScale:
      return cached.pixels
  let
    side = ArrowCanvas * renderScale
    centre = float32(side) / 2
    # aim increases counter-clockwise on screen; screen y is down, so draw at
    # -theta to match aimVector (the soldier cache's own convention).
    angle = float(r) * 2.0 * PI / float(ArrowRotations)
    s = float32(renderScale)
  var canvas = newImage(side, side)
  let unit =
    translate(vec2(centre, centre)) *
    rotate(float32(-angle)) *
    scale(vec2(s, s))
  var shaft = newPath()
  shaft.rect(rect(vec2(float32(-ArrowShaftPx) / 2, -1), vec2(float32(ArrowShaftPx), 2)))
  canvas.fillPath(shaft, rgba(214, 206, 186, 255).color, unit)
  var head = newPath()
  head.moveTo(vec2(float32(ArrowShaftPx) / 2, -3))
  head.lineTo(vec2(float32(ArrowShaftPx) / 2 + 4, 0))
  head.lineTo(vec2(float32(ArrowShaftPx) / 2, 3))
  head.closePath()
  canvas.fillPath(head, rgba(214, 218, 228, 255).color, unit)
  var fletch = newPath()
  fletch.moveTo(vec2(float32(-ArrowShaftPx) / 2, -3))
  fletch.lineTo(vec2(float32(-ArrowShaftPx) / 2 + float32(ArrowFletchPx), 0))
  fletch.lineTo(vec2(float32(-ArrowShaftPx) / 2, 3))
  fletch.closePath()
  canvas.fillPath(fletch, rgba(206, 108, 72, 255).color, unit)
  let pixels = soldierCanvasToPixels(canvas)
  arrowRotCache[r].add((scale: renderScale, pixels: pixels))
  pixels
