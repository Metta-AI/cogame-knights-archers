...
## wasm bundle. Clients read `window.KAZ_WIRE` and keep their old literals
...
const WireConstantsJs* =
  ...
  "window.KAZ_WIRE={speeds:[0.5," & jsIntArray(PlaybackSpeeds)[1..^1] &
  ",fps:" & $TargetFps &
...
const WireConstantsMarker* = "<!-- WIRE_CONSTANTS -->"
  ...
  ## (before any script that reads window.KAZ_WIRE).
