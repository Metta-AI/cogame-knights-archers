...
proc defaultGameConfig*(): GameConfig =
  ...
  GameConfig(
    ...
    fogOfWar: false,
    roles: @[],
    zombieHp: DefaultZombieHp,
    zombieSpeed: DefaultZombieSpeed,
    zombieReach: DefaultZombieReach,
    zombieLungePx: DefaultZombieLungePx,
    zombieStuckTicks: DefaultZombieStuckTicks,
    zombieSpawnX: DefaultZombieSpawnX,
    gateX: DefaultGateX,
    spawnStartPerMille: DefaultSpawnStartPerMille,
    spawnMaxPerMille: DefaultSpawnMaxPerMille,
    spawnSaturateTicks: DefaultSpawnSaturateTicks,
    spawnCapAlive: DefaultSpawnCapAlive,
    knightReach: DefaultKnightReach,
    knightArcBrads: DefaultKnightArcBrads,
    knightDamage: DefaultKnightDamage,
    knightCooldown: DefaultKnightCooldown,
    swingTicks: DefaultSwingTicks,
    archerSpeedPct: DefaultArcherSpeedPct,
    arrowSpeed: DefaultArrowSpeed,
    arrowRange: DefaultArrowRange,
    arrowLifeTicks: DefaultArrowLifeTicks,
    arrowDamage: DefaultArrowDamage,
    arrowCooldown: DefaultArrowCooldown,
    arrowHitRadius: DefaultArrowHitRadius,
    archerRange: DefaultArcherRange,
    roleStandoff: DefaultRoleStandoff,
    screenStandoff: DefaultScreenStandoff,
    archerPanicPx: DefaultArcherPanicPx,
    closeCallPx: DefaultCloseCallPx,
    clearBonus: DefaultClearBonus,
    roundTarget: DefaultRoundTarget,
    creditEpsilonPerMille: DefaultCreditEpsilonPerMille
  )
...
proc readConfigInt(node: JsonNode, name: string, value: var int) =
  ...
  let item = node[name]
  if item.kind != JInt:
    raise newException(KazError, "Config field " & name & " must be an integer.")
  value = item.getInt()
...
proc readConfigBool(node: JsonNode, name: string, value: var bool) =
  ...
  let item = node[name]
  if item.kind != JBool:
    raise newException(KazError, "Config field " & name & " must be a boolean.")
  value = item.getBool()
...
proc readConfigString(node: JsonNode, name: string, value: var string) =
  ...
  if item.kind != JString:
    raise newException(KazError, "Config field " & name & " must be a string.")
  value = item.getStr()
...
proc readConfigSlots(node: JsonNode, slots: var seq[PlayerSlotConfig]) =
  ...
  if items.kind != JArray:
    raise newException(KazError, "Config field slots must be an array.")
  slots.setLen(0)
...
proc readConfigPlayers(node: JsonNode, slots: var seq[PlayerSlotConfig]) =
  ...
  if items.kind != JArray:
    raise newException(KazError, "Config field players must be an array.")
...
proc readConfigTokens(
  ...
  if items.kind != JArray:
    raise newException(KazError, "Config field tokens must be an array.")
  if items.len > MaxPlayers:
    raise newException(
      KazError,
...
proc readConfigPerks(node: JsonNode, config: var GameConfig) =
  ...
  if perks.kind != JObject:
    raise newException(KazError, "Config field perks must be an object.")
...
proc readConfigPerkMods(node: JsonNode, config: var GameConfig) =
  ...
  if mods.kind != JObject:
    raise newException(KazError, "Config field perkMods must be an object.")
  for key in mods.keys:
    if key notin ["armorHp", "scopeAim", "grenadeRange", "thrusterSpeed",
        "luckChance", "luckDamage"]:
      raise newException(
        KazError, "Config field perkMods has unknown key " & key & ".")
  ...
  if config.perkMods.luckDamage < 1:
    raise newException(
      KazError, "Config field perkMods.luckDamage must be at least 1.")
...
proc readConfigHandicaps(node: JsonNode, config: var GameConfig) =
  ...
  if handicaps.kind != JObject:
    raise newException(KazError, "Config field handicaps must be an object.")
...
proc readConfigRoles(node: JsonNode, config: var GameConfig) =
  ## `roles`: one entry per seat, "knight" or "archer". Absent means the
  ## shipped default — the first half of the seats are knights, the rest
  ## archers — so a config that only sets num_agents still plays.
  if not node.hasKey("roles"):
    return
  let items = node["roles"]
  if items.kind != JArray:
    raise newException(KazError, "Config field roles must be an array.")
  var roles: seq[string] = @[]
  for i, item in items.getElems():
    if item.kind != JString:
      raise newException(
        KazError, "Config field roles[" & $i & "] must be a string.")
    let role = item.getStr().strip().toLowerAscii()
    if role notin [RoleKnight, RoleArcher]:
      raise newException(
        KazError,
        "Config field roles[" & $i & "] must be " & RoleKnight & " or " &
          RoleArcher & "; got " & role & ".")
    roles.add(role)
  config.roles = roles
...
proc readConfigRegimes(node: JsonNode, config: var GameConfig) =
  ...
  if items.kind != JArray:
    raise newException(KazError, "Config field regimes must be an array.")
  if items.len < 1 or items.len > 4:
    raise newException(
      KazError, "Config field regimes must have 1..4 entries.")
...
proc validate(config: GameConfig) =
  ...
  if config.motionScale <= 0:
    raise newException(KazError, "Config field motionScale must be positive.")
  if config.frictionDen <= 0:
    raise newException(KazError, "Config field frictionDen must be positive.")
  if config.minPlayers < 1:
    raise newException(KazError, "Config field minPlayers must be at least 1.")
  if config.teams notin [2, 4]:
    raise newException(KazError, "Config field teams must be 2 or 4.")
  ...
  if config.minPlayers > MaxPlayers:
    raise newException(KazError, "can't do more than 8 players.")
  if config.lives < 1:
    raise newException(KazError, "Config field lives must be at least 1.")
  if config.hitPoints < 1:
    raise newException(KazError, "Config field hitPoints must be at least 1.")
  if config.gunRange <= 0:
    raise newException(KazError, "Config field gunRange must be positive.")
  if config.fireWindupTicks < 0:
    raise newException(KazError, "Config field fireWindupTicks must not be negative.")
  if config.carrierSpeedPct <= 0 or config.carrierSpeedPct > 100:
    raise newException(KazError, "Config field carrierSpeedPct must be 1..100.")
  if config.playerBouncePct < 0 or config.playerBouncePct > 100:
    raise newException(KazError, "Config field playerBouncePct must be 0..100.")
  if config.aimTurnRate < 1:
    raise newException(KazError, "Config field aimTurnRate must be at least 1.")
  if config.visionConeDeg < 0 or config.visionConeDeg > 180:
    raise newException(KazError, "Config field visionConeDeg must be between 0 and 180.")
  if config.puddleDamagePct < 0 or config.puddleDamagePct > 100:
    raise newException(KazError, "Config field puddleDamagePct must be 0..100.")
  ...
  if config.visionBubble < 0:
    raise newException(KazError, "Config field visionBubble must be non-negative.")
  ...
  if config.startWaitTicks < 0:
    raise newException(KazError, "Config field startWaitTicks must be non-negative.")
  if config.lobbyJoinTimeoutTicks < 0:
    raise newException(KazError, "Config field lobbyJoinTimeoutTicks must be non-negative.")
  if config.respawnTicks < 0 or config.fireCooldownTicks < 0:
    raise newException(KazError, "Timer config fields must not be negative.")
  ...
  if config.barrageMaxPerSec > 0:
    ...
    if config.barrageStartSec < 1:
      raise newException(
        KazError, "Config field barrageStartSec must be at least 1.")
    if config.barrageSaturateSec < 1:
      raise newException(
        KazError, "Config field barrageSaturateSec must be at least 1.")
  if config.loadout notin [LoadoutKaz, LoadoutPaintball]:
    raise newException(
      KazError,
      "Config field loadout must be " & LoadoutKaz & " or " &
        LoadoutPaintball & "; got " & config.loadout & "."
    )
  if config.cogsPerTeam < 1 or config.cogsPerTeam > 8:
    raise newException(KazError, "Config field cogsPerTeam must be 1..8.")
  if config.paintTile < 4:
    raise newException(KazError, "Config field paintTile must be at least 4.")
  if config.hillRadiusTiles < 0:
    raise newException(
      KazError, "Config field hillRadiusTiles must not be negative.")
  ...
  if config.hillDecisiveTicks < 1:
    raise newException(
      KazError, "Config field hillDecisiveTicks must be at least 1.")
  if config.paintSpeedOwnPct < 1 or config.paintSpeedEnemyPct < 1:
    raise newException(
      KazError, "Config field paintSpeed*Pct must be positive.")
  if config.paintHealTicks < 1:
    raise newException(
      KazError, "Config field paintHealTicks must be at least 1.")
  if config.sprayDamage < 1:
    raise newException(
      KazError, "Config field sprayDamage must be at least 1.")
  if config.regimes.len < 1 or config.regimes.len > 4:
    raise newException(
      KazError, "Config field regimes must have 1..4 entries.")
  if config.turnTicks < 1:
    raise newException(KazError, "Config field turnTicks must be at least 1.")
  if config.turnBudgetMs < 1 or config.attempt1Ms < 1 or config.retryMs < 1:
    raise newException(
      KazError, "Config LLM deadline fields must be positive.")
  ...
  if config.turnSpacingMs < 0:
    raise newException(
      KazError, "Config field turnSpacingMs must not be negative.")
  if config.wallClockBudgetSeconds < 1:
    raise newException(
      KazError, "Config field wallClockBudgetSeconds must be positive.")
  # --- knights-archers bounds. Every one of these is read by the wasm viewer
  # out of the replay config and used to re-derive the horde, so a value the
  # native server would accept and the viewer could not is a divergence.
  if config.zombieHp < 1:
    raise newException(KazError, "Config field zombieHp must be at least 1.")
  if config.zombieSpeed < 1:
    raise newException(KazError, "Config field zombieSpeed must be positive.")
  if config.zombieReach < 1:
    raise newException(KazError, "Config field zombieReach must be positive.")
  if config.zombieLungePx < 0:
    raise newException(
      KazError, "Config field zombieLungePx must not be negative.")
  if config.zombieStuckTicks < 1:
    raise newException(
      KazError, "Config field zombieStuckTicks must be positive.")
  if config.spawnStartPerMille < 0 or config.spawnStartPerMille > 1000:
    raise newException(
      KazError, "Config field spawnStartPerMille must be 0..1000.")
  if config.spawnMaxPerMille < config.spawnStartPerMille or
      config.spawnMaxPerMille > 1000:
    raise newException(
      KazError,
      "Config field spawnMaxPerMille must be spawnStartPerMille..1000.")
  if config.spawnSaturateTicks < 1:
    raise newException(
      KazError, "Config field spawnSaturateTicks must be positive.")
  if config.spawnCapAlive < 1 or config.spawnCapAlive > MaxZombies:
    raise newException(
      KazError, "Config field spawnCapAlive must be 1.." & $MaxZombies & ".")
  if config.knightReach < 1:
    raise newException(KazError, "Config field knightReach must be positive.")
  if config.knightArcBrads < 1 or config.knightArcBrads > 63:
    raise newException(
      KazError, "Config field knightArcBrads must be 1..63.")
  if config.knightDamage < 1:
    raise newException(KazError, "Config field knightDamage must be positive.")
  if config.knightCooldown < 1:
    raise newException(
      KazError, "Config field knightCooldown must be positive.")
  if config.swingTicks < 1:
    raise newException(KazError, "Config field swingTicks must be positive.")
  if config.archerSpeedPct < 1 or config.archerSpeedPct > 200:
    raise newException(
      KazError, "Config field archerSpeedPct must be 1..200.")
  if config.arrowSpeed < 1:
    raise newException(KazError, "Config field arrowSpeed must be positive.")
  if config.arrowLifeTicks < 1:
    raise newException(
      KazError, "Config field arrowLifeTicks must be positive.")
  if config.arrowDamage < 1:
    raise newException(KazError, "Config field arrowDamage must be positive.")
  if config.arrowCooldown < 1:
    raise newException(
      KazError, "Config field arrowCooldown must be positive.")
  if config.arrowHitRadius < 1:
    raise newException(
      KazError, "Config field arrowHitRadius must be positive.")
  if config.roundTarget < 1:
    raise newException(KazError, "Config field roundTarget must be positive.")
  if config.clearBonus < 0:
    raise newException(
      KazError, "Config field clearBonus must not be negative.")
  if config.creditEpsilonPerMille < 0 or config.creditEpsilonPerMille >= 1000:
    raise newException(
      KazError, "Config field creditEpsilonPerMille must be 0..999.")
  if config.roles.len > 0 and config.numAgents > 0 and
      config.roles.len != config.numAgents:
    raise newException(
      KazError,
      "Config field roles must have one entry per seat (num_agents = " &
        $config.numAgents & "); got " & $config.roles.len & ".")
  if config.paintBuff and not config.floorPaint:
    raise newException(
      KazError, "Config field paintBuff requires floorPaint.")
  if config.hill and not config.floorPaint:
    raise newException(
      KazError, "Config field hill requires floorPaint.")
  if config.numAgents < 0:
    raise newException(KazError, "Config field num_agents must not be negative.")
  if config.slots.len > MaxPlayers:
    raise newException(KazError, "Config field slots cannot have more than 8 entries.")
...
proc update*(config: var GameConfig, jsonText: string) =
  ...
  try:
    ...
    raise newException(KazError, "Could not parse config JSON: " & e.msg)
  if node.kind != JObject:
    raise newException(KazError, "Config must be a JSON object.")
  ...
  node.readConfigBool("fogOfWar", config.fogOfWar)
  node.readConfigRoles(config)
  node.readConfigInt("zombieHp", config.zombieHp)
  node.readConfigInt("zombieSpeed", config.zombieSpeed)
  node.readConfigInt("zombieReach", config.zombieReach)
  node.readConfigInt("zombieLungePx", config.zombieLungePx)
  node.readConfigInt("zombieStuckTicks", config.zombieStuckTicks)
  node.readConfigInt("zombieSpawnX", config.zombieSpawnX)
  node.readConfigInt("gateX", config.gateX)
  node.readConfigInt("spawnStartPerMille", config.spawnStartPerMille)
  node.readConfigInt("spawnMaxPerMille", config.spawnMaxPerMille)
  node.readConfigInt("spawnSaturateTicks", config.spawnSaturateTicks)
  node.readConfigInt("spawnCapAlive", config.spawnCapAlive)
  node.readConfigInt("knightReach", config.knightReach)
  node.readConfigInt("knightArcBrads", config.knightArcBrads)
  node.readConfigInt("knightDamage", config.knightDamage)
  node.readConfigInt("knightCooldown", config.knightCooldown)
  node.readConfigInt("swingTicks", config.swingTicks)
  node.readConfigInt("archerSpeedPct", config.archerSpeedPct)
  node.readConfigInt("arrowSpeed", config.arrowSpeed)
  node.readConfigInt("arrowRange", config.arrowRange)
  node.readConfigInt("arrowLifeTicks", config.arrowLifeTicks)
  node.readConfigInt("arrowDamage", config.arrowDamage)
  node.readConfigInt("arrowCooldown", config.arrowCooldown)
  node.readConfigInt("arrowHitRadius", config.arrowHitRadius)
  node.readConfigInt("archerRange", config.archerRange)
  node.readConfigInt("roleStandoff", config.roleStandoff)
  node.readConfigInt("screenStandoff", config.screenStandoff)
  node.readConfigInt("archerPanicPx", config.archerPanicPx)
  node.readConfigInt("closeCallPx", config.closeCallPx)
  node.readConfigInt("clearBonus", config.clearBonus)
  node.readConfigInt("roundTarget", config.roundTarget)
  node.readConfigInt("creditEpsilonPerMille", config.creditEpsilonPerMille)
  node.readConfigRegimes(config)
  if node.hasKey("mapSpec"):
    if node["mapSpec"].kind != JObject:
      raise newException(KazError, "Config field mapSpec must be an object.")
  ...
  ## map-wide default.
  let mapMeta = resolveKazMapMetadata(config)
...
proc configJson*(config: GameConfig): string =
  ...
  # different game.
  let paintballOn = config.loadout != LoadoutKaz or config.floorPaint or
  ...
  if paintballOn:
    ...
    node["fogOfWar"] = %config.fogOfWar
    node["roles"] = (block:
      var arr = newJArray()
      for role in config.roles:
        arr.add(%role)
      arr)
    node["zombieHp"] = %config.zombieHp
    node["zombieSpeed"] = %config.zombieSpeed
    node["zombieReach"] = %config.zombieReach
    node["zombieLungePx"] = %config.zombieLungePx
    node["zombieStuckTicks"] = %config.zombieStuckTicks
    node["zombieSpawnX"] = %config.zombieSpawnX
    node["gateX"] = %config.gateX
    node["spawnStartPerMille"] = %config.spawnStartPerMille
    node["spawnMaxPerMille"] = %config.spawnMaxPerMille
    node["spawnSaturateTicks"] = %config.spawnSaturateTicks
    node["spawnCapAlive"] = %config.spawnCapAlive
    node["knightReach"] = %config.knightReach
    node["knightArcBrads"] = %config.knightArcBrads
    node["knightDamage"] = %config.knightDamage
    node["knightCooldown"] = %config.knightCooldown
    node["swingTicks"] = %config.swingTicks
    node["archerSpeedPct"] = %config.archerSpeedPct
    node["arrowSpeed"] = %config.arrowSpeed
    node["arrowRange"] = %config.arrowRange
    node["arrowLifeTicks"] = %config.arrowLifeTicks
    node["arrowDamage"] = %config.arrowDamage
    node["arrowCooldown"] = %config.arrowCooldown
    node["arrowHitRadius"] = %config.arrowHitRadius
    node["archerRange"] = %config.archerRange
    node["roleStandoff"] = %config.roleStandoff
    node["screenStandoff"] = %config.screenStandoff
    node["archerPanicPx"] = %config.archerPanicPx
    node["closeCallPx"] = %config.closeCallPx
    node["clearBonus"] = %config.clearBonus
    node["roundTarget"] = %config.roundTarget
    node["creditEpsilonPerMille"] = %config.creditEpsilonPerMille
