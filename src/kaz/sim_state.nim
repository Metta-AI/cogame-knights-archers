...
proc spawnAimBrads*(gameMap: KazMap, team: Team): int =
...
proc spawnFlipH*(gameMap: KazMap, team: Team): bool =
...
proc gameHash*(sim: SimServer): uint64 =
  ...
  # --- knights-archers state, APPENDED after every existing mix so the
  # ordering of the inherited fields stays stable. All of it is gameplay state
  # the wasm viewer re-derives from the recorded masks, so all of it is
  # hashed. `gateDist` is DERIVED (a pure function of position and the
  # installed field) and therefore deliberately absent.
  result.mixHashInt(sim.zombies.len)
  for zombie in sim.zombies:
    result.mixHashInt(zombie.id)
    result.mixHashInt(zombie.ux)
    result.mixHashInt(zombie.uy)
    result.mixHashInt(zombie.hp)
    result.mixHashInt(zombie.lungeTarget)
    result.mixHashInt(zombie.stuckTicks)
    result.mixHashInt(zombie.stuckRotTicks)
    result.mixHashBool(zombie.alive)
    result.mixHash(uint64(zombie.hitByMask))
  result.mixHashInt(sim.zombieNextId)
  result.mixHashInt(sim.spawnAcc)
  result.mixHashInt(sim.aliveZombies)
  result.mixHashInt(sim.zombiesSpawned)
  result.mixHashInt(sim.zombiesKilled)
  result.mixHashInt(sim.waveKillsSoFar)
  result.mixHashInt(sim.arrows.len)
  for arrow in sim.arrows:
    result.mixHashInt(arrow.ux)
    result.mixHashInt(arrow.uy)
    result.mixHashInt(arrow.vx)
    result.mixHashInt(arrow.vy)
    result.mixHashInt(arrow.owner)
    result.mixHashInt(arrow.ticksLeft)
  result.mixHashInt(sim.heroKills.len)
  for i in 0 ..< sim.heroKills.len:
    result.mixHashInt(sim.heroKills[i])
  for kills in sim.heroHits:
    result.mixHashInt(kills)
  for shots in sim.heroShots:
    result.mixHashInt(shots)
  result.mixHashInt(sim.minGateDist)
  result.mixHashInt(sim.minGateTick)
  result.mixHashInt(sim.lastCloseCallTick)
  result.mixHashInt(sim.wavesCleared)
  result.mixHashInt(sim.waveCasualty)
