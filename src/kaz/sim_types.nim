...
const
  GameName* = "knights-archers"
  GameVersion* = "1"  ## GV1 (horde rule): KNIGHTS ARCHERS ZOMBIES, v1.
    ## The first rules version of this coworld. Four heroes — two melee
    ## knights and two ranged archers — hold the west gate against a horde
    ## marching in from the east breach. A zombie that reaches the gate, or
    ## that touches ANY hero, ends the wave for everybody. The horde, the
    ## arrows and the swings are all new hashed sim state appended after the
    ## inherited mixes, so no replay of the starter's rules re-simulates
    ## here and none is expected to.
    ##
    ## Inherited history (coworld-ctf GV44 and earlier) below, kept because
    ## the engine underneath is that engine.
    ##
  ...
  MaceLengthPx* = 34           ## top-down gun master length on the map (stock-tip to
                              ## muzzle, along the aim ray).
  MaceGripPx* = -13            ## gun stock-tip offset from the body center, along
                              ## aim (negative = stock sits behind the hub so the
                              ## barrel reaches out front, marker straddling the cog).
  WeaponRightPx* = 10            ## the marker is held at the cog's RIGHT: barrel
  ...
  WeaponGlowRadius* = 0.6        ## px blur (master-frame): tiny, so the rim is CRISP —
                              ## an outline stroke, not a soft glow.
  WeaponGlowSpread* = 1.0        ## px the silhouette expands before blurring — this is
                              ## the outline WIDTH that sticks out past the gun edge.
  WeaponGlowAlpha* = 95'u8       ## faint warm outline (0..255), reads as a subtle stroke.
  BowLengthPx* = 22     ## the held spray can's length on the map, along the
                              ## aim ray. Shorter than the marker (MaceLengthPx):
                              ## a can is a fistful, and the silhouette difference
                              ## is what tells a viewer WHICH weapon a cog holds.
  BowGripPx* = -6       ## can tail offset from the body center along aim.
                              ## Less negative than MaceGripPx so the short can
  ...
  LoadoutKaz* = "ctf"           ## the starter's loadout: pickups, gun, hearts.
  ...
  MaxSeatIdRunes* = 16          ## `cogs[].id` cap for a hero alias, in RUNES.

  # --- knights-archers: the horde, the heroes, the weapons -----------------
  # Every number here is a DEFAULT for the matching GameConfig field, so a
  # manifest variant can retune difficulty without touching the binary. All
  # of it is integer: the horde is hashed state re-derived tick for tick by
  # the wasm viewer, and Nim's `int` is 32-bit under --cpu:wasm32.
  RoleKnight* = "knight"
  RoleArcher* = "archer"
  MaxHeroSeats* = 8             ## hard cap on seats, for the fixed-layout
                                ## per-seat arrays on SimServer. The shipped
                                ## variants all seat exactly four.
  MaxZombies* = 64              ## hard array cap on the zombie list.
  MaxArrows* = 64               ## hard array cap on the in-flight arrows.
  HordeNavCell* = 12            ## px side of one zombieField cell.
                                ## Sized to the ARENA, not to a round number:
                                ## its corridors are ~26 px wide for a 13 px
                                ## body, so at a 34 px cell (the design note's
                                ## first figure) there is no usable node
                                ## anywhere inside a gap between two obstacles
                                ## and two cells the BFS calls adjacent are
                                ## separated by stone. Measured on that grid a
                                ## marching zombie pressed west into a wall
                                ## for two thousand ticks. At 12 px every
                                ## 26 px corridor contains a node with the
                                ## full body's clearance -- the same figure and
                                ## the same reason as control.nim's NavCell.
  MinSpawnRows* = 120           ## map-install assertion on spawnRows.len.
  DefaultZombieHp* = 2          ## one mace blow, or two arrows.
  DefaultZombieSpeed* = 384     ## motion units/tick = 1.5 px/tick = 36 px/s.
  DefaultZombieReach* = 26      ## px: this close to a hero centre kills it.
  DefaultZombieLungePx* = 90    ## px: inside this a zombie charges the hero.
  DefaultZombieStuckTicks* = 24 ## blocked ticks before the quarter-turn escape.
  DefaultZombieRotTicks* = 12   ## ticks the escape rotation holds.
  DefaultZombieSpawnX* = 1178   ## the breach column's spawn line.
  DefaultGateX* = 40            ## the gate line; a zombie at or past it wins.
  DefaultSpawnStartPerMille* = 12   ## zombies/1000 ticks at wave start.
  DefaultSpawnMaxPerMille* = 50     ## zombies/1000 ticks at saturation.
  DefaultSpawnSaturateTicks* = 1920 ## ticks from wave start to that rate.
  DefaultSpawnCapAlive* = 40    ## no spawn fires at or above this many alive.
  ...
  DefaultArrowRange* = 528      ## px an arrow flies before it drops.
  DefaultArrowLifeTicks* = 44   ## 44 x 12 px = 528 px.
  DefaultArrowDamage* = 1       ## two arrows kill.
  DefaultArrowCooldown* = 12    ## ticks between shots (0.5 s).
  DefaultArrowHitRadius* = 14   ## px around the swept segment that connects.
  DefaultArcherRange* = 460     ## px the control layer stands off at on focus.
  DefaultRoleStandoff* = 300    ## px an archer backs off to on intercept.
  DefaultScreenStandoff* = 120  ## px in front of the leader `screen` stands.
  DefaultArcherPanicPx* = 150   ## px inside which `phalanx` archers fall back.
  DefaultCloseCallPx* = 200     ## gate distance that raises a `closecall`.
  DefaultClearBonus* = 20       ## team value banked for a cleared wave.
  DefaultRoundTarget* = 90      ## team value worth 1.0 for one wave.
  DefaultCreditEpsilonPerMille* = 4
    ## The whole per-seat kill-credit range, in permille of a point: 0.004,
    ## deliberately SMALLER than one extra team kill (1/180 = 0.005556), so
    ## the ordering is lexicographic — squad kills first, personal credit
    ## only as a tie-break. tests/test_scoring.nim asserts the bound.
  AimUnitScale* = 1024
  ...
  EndRuleBreach* = "breach"     ## a zombie reached the gate line.
  EndRuleCasualty* = "casualty" ## a zombie touched a hero; one death ends it.
...
## process by loadKazMap (driven by config.mapPath) BEFORE any sim, mask,
...
## initialized to the default arena so tools that never call loadKazMap
...
type
  ...
  KazMap* = object
    ...
    homeRotation*: int         ## GV44: per-episode rotation of TEAM -> HOME
                               ...
                               ## `resolveKazMapMetadata`, so it is a pure
  ...
  MapGenOverrides* = object
    ...
    baseDepth*: int        ## home anchor depth permille (see KazMap.
  ...
  GameConfig* = object
    ...
    loadout*: string              ## LoadoutKaz (default) or LoadoutPaintball.
    ...
    # --- knights-archers (horde defence). APPENDED at the end of the object:
    # GameConfig rides the flatty replay keyframe positionally, so new fields
    # go last and never in the middle.
    fogOfWar*: bool               ## false = every seat sees the whole board.
    roles*: seq[string]           ## per seat: RoleKnight or RoleArcher.
    zombieHp*: int
    zombieSpeed*: int
    zombieReach*: int
    zombieLungePx*: int
    zombieStuckTicks*: int
    zombieSpawnX*: int
    gateX*: int
    spawnStartPerMille*: int
    spawnMaxPerMille*: int
    spawnSaturateTicks*: int
    spawnCapAlive*: int
    knightReach*: int
    knightArcBrads*: int
    knightDamage*: int
  ...
  HitFlashFx* = object
    ...
    x*, y*: int                ## the impact site, for a flash with no body
                               ## behind it (an arrow's wall thunk). Ignored
                               ## whenever playerIndex >= 0.
  ...
  SimEventKind* = enum
    ...
    Directive   ## NEW: a seat's directive for a turn; weapon = the source
                ...
                ## content = the note.
    Swing       ## NEW: a knight threw a mace swing; headingBrads = the aim
                ## it was locked at.
    ZombieSpawn ## NEW: one zombie entered at the breach; amount = its id.
    ZombieHit   ## NEW: a hero damaged a zombie; amount = damage, hp = hp left.
    ZombieKill  ## NEW: a zombie died; source = the crediting hero.
    HeroDeath   ## NEW: a zombie touched a hero; amount = the zombie's id.
    Breach      ## NEW: a zombie reached the gate; amount = its id.
    CloseCall   ## NEW: the wave's closest call fell inside closeCallPx;
                ## amount = the gate distance in px, hp = the zombie's id.
  ...
  Zombie* = object
    ## One walking dead. Positions are in MOTION UNITS (px * MotionScale), so
    ## a 1.5 px/tick march is exact integer arithmetic on every target; the
    ## pixel centre is `ux div MotionScale`.
    id*: int                   ## stable within a wave; ordering is total.
    ux*, uy*: int              ## body centre, motion units.
    hp*: int
    lungeTarget*: int          ## seat index being charged, -1 = marching.
    stuckTicks*: int           ## consecutive blocked ticks.
    stuckRotTicks*: int        ## ticks left of the quarter-turn escape.
    alive*: bool
    hitByMask*: uint32         ## cog indices whose CURRENT swing activation
                               ## has already damaged this zombie: one hit per
                               ## victim per activation, even across the
                               ## swing's four lit ticks.

  Arrow* = object
    ## One arrow in flight. Same motion-unit convention as Zombie.
    ux*, uy*: int
    vx*, vy*: int              ## velocity, motion units per tick.
    owner*: int                ## the archer's cog index.
    ticksLeft*: int
  ...
  SimServer* = object
    ...
    llmTurns*: array[MaxHeroSeats, int]
                               ## per seat: turns whose directive came from an
                               ## LLM. Sized to MaxHeroSeats rather than to
                               ## num_agents so the flatty keyframe layout is
                               ## fixed however many seats a variant declares.
    fallbackTurns*: array[MaxHeroSeats, int]
                               ## per seat: turns that fell back to scripted.
    seatNames*: array[MaxHeroSeats, string]
                               ## real policy names, SPECTATOR SIDE ONLY.
    seatPolicyKind*: array[MaxHeroSeats, string]  ## "llm" | "scripted".
    ...
    # --- knights-archers state. APPENDED last: SimServer rides the flatty
    # replay keyframe positionally.
    zombies*: seq[Zombie]      ## the horde. HASHED.
    arrows*: seq[Arrow]        ## arrows in flight. HASHED.
    zombieNextId*: int         ## next zombie id in this wave. HASHED.
    spawnAcc*: int             ## per-mille spawn accumulator. HASHED.
    aliveZombies*: int         ## live count, recomputed every tick. HASHED.
    zombiesSpawned*: int       ## spawned this wave. HASHED.
    zombiesKilled*: int        ## killed this EPISODE. HASHED.
    waveKillsSoFar*: int       ## killed this WAVE. HASHED.
    minGateDist*: int          ## closest a zombie came to the gate, px. HASHED.
    minGateTick*: int          ## the tick it happened. HASHED.
    lastCloseCallTick*: int    ## throttle for the `closecall` beat. HASHED.
    wavesCleared*: int         ## waves that ended full_time. HASHED.
    waveCasualty*: int         ## seat whose hero fell, -1 = none. HASHED.
    breachZombie*: int         ## zombie id that reached the gate, -1 = none.
    zombieField*: seq[int]     ## BFS gate distance in HordeNavCell steps, -1
                               ## unreachable. Installed once with the map, so
                               ## it is derived, not hashed.
    zombieFieldW*, zombieFieldH*: int
    zombieFieldNodeX*, zombieFieldNodeY*: seq[int]
                               ## per cell: the walkable pixel a zombie steers
                               ## at, -1 when the cell has none. The cell
                               ## CENTRE is not usable as a node — the arena's
                               ## corridors are ~26 px wide inside a 34 px
                               ## cell, so a centre lands in stone often
                               ## enough that a centre-only field reports
                               ## whole regions unreachable and the horde
                               ## walks into a wall and stays there.
    spawnRows*: seq[int]       ## every clear spawn y on the breach column.
    spawnGateDist*: int        ## field distance at the spawn column, px.
    heroKills*: seq[int]       ## per COG: zombies whose last damage was its.
    heroHits*: seq[int]        ## per COG: damaging connections.
    heroShots*: seq[int]       ## per COG: arrows loosed / swings thrown.
    heroDeathTick*: seq[int]   ## per COG: tick it fell, -1 = alive.
    waveTicksLog*: seq[int]    ## per finished wave.
...
proc teamCount*(gameMap: KazMap): int =
...
proc teams*(gameMap: KazMap): Slice[Team] =
...
proc parsePerk*(text: string): Perk =
  ## Parses one authored perk name; raises KazError on an unknown name.
  for perk in Perk:
    if PerkNames[perk] == text:
      return perk
  raise newException(KazError, "Unknown perk name: " & text)
