...
proc endzoneTints(gameMap: KazMap): seq[EndzoneTint] =
...
proc renderArenaRgbaPair*(
  ...
  # THE GATE AND THE BREACH are baked FLOOR ART, not sprites: a portcullis
  # strip over the gate column (Red's home column, the west edge) and a torn,
  # blood-dark strip over the breach column (the east edge, where the dead
  # walk in). Baked here, the same way the starter baked the endzone paint, so
  # the geometry of the game is legible with the HUD off — and so no sprite
  # pool, no object budget and no hashed state pays for it.
  block gateAndBreach:
    let
      gate = gameMap.captureZone(Red)
      breach = gameMap.captureZone(Blue)
      barPitch = 14 * scale
      barHalf = max(1, 3 * scale)
    for y in 0 ..< oh:
      for x in 0 ..< ow:
        let
          lx = x div scale
          offset = (y * ow + x) * 4
        template stamp(colr, colg, colb, alpha: int) =
          let a = alpha
          for c in 0 .. 2:
            let src = (if c == 0: colr elif c == 1: colg else: colb)
            result.hot[offset + c] =
              uint8((src * a + result.hot[offset + c].int * (255 - a)) div 255)
            result.cold[offset + c] =
              uint8((src * a + result.cold[offset + c].int * (255 - a)) div 255)
        if lx >= gate.xLo and lx <= gate.xHi:
          # A cold slate threshold with iron bars: unmistakably a doorway the
          # squad is standing in front of.
          stamp(46, 50, 58, 150)
          if (x mod barPitch) < barHalf * 2:
            stamp(96, 100, 110, 210)
...
proc loadMapLayers*(gameMap: KazMap, withEndzoneGlow = true):
  ...
  ## THE GATE AND THE BREACH, baked into the 1x floor art exactly as the
  ## supersampled pass bakes them (cosmetic only — the walk/wall collision
  ## masks above are untouched), so the two columns read the same at every
  ## render scale.
  block gateAndBreach:
    let
      gate = gameMap.captureZone(Red)
    ...
    for y in 0 ..< h:
      for x in 0 ..< w:
        if x >= gate.xLo and x <= gate.xHi:
          var px = result.mapImage[x, y].rgba
          px = overTint(px, rgba(46, 50, 58, 150))
          if (x mod 14) < 6:
          ...
          result.mapImage[x, y] = overTint(
            ...
            rgba(58, 22, 24, uint8(130 + int(h2 mod 60'u32)))).rgbx

proc coldEndzoneMapRgba*(gameMap: KazMap): seq[uint8] =
