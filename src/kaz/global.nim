...
const
  ...
  BoardObjectPools = [
    ...
    ("zombies", ZombieObjectBase, MaxZombies),
    ("arrows", ArrowObjectBase, MaxArrows),
...
proc endzoneStripBox(gameMap: KazMap, team: Team):
...
proc rigMaceSpriteId(team: Team, aimStep: int): int =
  wireSpriteId(RigGunSpriteBase + ord(team) * RigSteps + aimStep)

proc rigBowSpriteId(team: Team, aimStep: int): int =
...
const
  ...
  SprayNozzleFwd = BowGripPx + BowLengthPx
    ## Where the paint actually LEAVES the can, ALONG the aim: the held can's
    ## tail sits BowGripPx along the aim and the can is BowLengthPx
  ...
  SprayNozzleRight = WeaponRightPx
    ## ...and PERPENDICULAR to the aim: the can is held at the cog's RIGHT, the
    ## same WeaponRightPx off the aim ray as the marker it replaces, so the nozzle
...
proc endzoneShapeToken(gameMap: KazMap, zone: CaptureZone): string =
...
proc addFlagSprites(
  ...
  ## THE HEART OBJECTIVE IS GONE with the mechanic it belonged to: there is no
  ## heart to plant, steal, carry or capture in a horde defence, so no banner,
  ## no planted gem, no carrier halo and no per-aim carried-heart bake is ever
  ## defined. The proc is kept as the one seam the two frame builders call, so
  ## a future objective has somewhere to live.
  discard sim
  discard spriteDefs
  discard packet
...
proc addHorde(
  ...
  ## The horde and the arrows in flight, emitted incrementally like every other
  ## object family: one object per live zombie in ID order and one per live
  ## arrow in creation order, so a live frame is at most 4 heroes + 40 zombies
  ## + 16 arrows of moving objects — well inside the budget the starter already
  ## carries. Sprites are defined lazily, so an all-quiet frame registers none.
  ##
  ## Zombies draw BELOW the heroes' z (a hero must never be hidden behind the
  ## thing that is about to kill it) and arrows draw above everything: they fly
  ## over walls and bodies alike.
  var index = 0
  for zombie in sim.zombies:
    ...
    if index >= MaxZombies:
      break
    let
      (zx, zy) = zombie.zombiePx()
      ## The shamble reads off the TICK, not off the zombie, so a lane of dead
      ## limps together rather than shimmering: it is a horde, not a crowd.
      frame = (sim.tickCount div 8) mod ZombieShambleFrames
      ## A zombie faces where it is going: toward its lunge target if it has
      ## one, else west toward the gate.
      heading =
        if zombie.lungeTarget >= 0 and zombie.lungeTarget < sim.players.len:
          bradsOfVector(
            sim.players[zombie.lungeTarget].x + CollisionW div 2 - zx,
            sim.players[zombie.lungeTarget].y + CollisionH div 2 - zy)
        else:
          AimBradsTurn div 2
      rot = soldierRotIndex(heading)
      spriteId = ZombieSpriteBase + frame * SoldierRotations + rot
    if spriteDefs.spriteDefinitionIndex(spriteId) < 0:
      packet.addBoardSpriteChanged(
        spriteDefs, spriteId, SoldierCanvas, SoldierCanvas,
        zombieRotPixels(rot, frame, boardScale),
        LabelZombie, native = boardScale)
    let objectId = ZombieObjectBase + index
    currentIds.add(objectId)
    packet.addBoardObject(
      objectId,
      zx - SoldierCanvas div 2,
      zy - SoldierCanvas div 2,
      zy - 1, MapLayerId, spriteId)
    inc index
  ...
  for arrow in sim.arrows:
    ...
    let
      (ax, ay) = arrow.arrowPx()
      rot = soldierRotIndex(bradsOfVector(arrow.vx, arrow.vy))
      spriteId = ArrowSpriteBase + rot
    if spriteDefs.spriteDefinitionIndex(spriteId) < 0:
      packet.addBoardSpriteChanged(
        spriteDefs, spriteId, ArrowCanvas, ArrowCanvas,
        arrowRotPixels(rot, boardScale), LabelArrow, native = boardScale)
    let objectId = ArrowObjectBase + arrowIndex
    currentIds.add(objectId)
    packet.addBoardObject(
      objectId,
      ax - ArrowCanvas div 2,
      ay - ArrowCanvas div 2,
...
proc addSprayPaintFlashes(
  ...
  for i in 0 ..< min(sim.sprayPaintFlashes.len, SprayPaintMaxFlashes):
    ...
    let
      ...
      # (0, 1) — screen-down — matching the held weapon's WeaponRightPx convention.
      (rx, ry) = (-uy, ux)
...
proc buildSpriteProtocolPlayerUpdates*(
  ...
  if sim.phase != Playing or playerIndex < 0 or
    ...
    ## THE HORDE IS IN EVERY SEAT FRAME. `fogOfWar` is false in every shipped
    ## variant: a cooperative squad that cannot see the board cannot divide the
    ## board, and the horde pressure bar is a public readout.
    sim.addHorde(nextState.spriteDefs, currentIds, result)
    sim.addMedKits(
      nextState.spriteDefs,
      currentIds,
      result,
...
proc addCogRigObjects(
  ...
  # a warm backlight glow so the dark art pops. Drawn ABOVE the head so it
  # always reads. A hero holds exactly one thing, decided by its ROLE and never
  # changing: a knight's long solid mace or an archer's short open bow. That
  # silhouette difference is how a spectator reads the roles at board scale
  # with no label at all. Both share one object slot.
  let
    holdsBow = sim.roleForSeat(player.joinOrder) != RoleKnight
    weaponSpriteId =
      if holdsBow: rigBowSpriteId(player.team, aimStep)
      else: rigMaceSpriteId(player.team, aimStep)
  if spriteDefs.spriteDefinitionIndex(weaponSpriteId) < 0:
    packet.addBoardSpriteChanged(
      spriteDefs, weaponSpriteId, RigCanvas, RigCanvas,
      (if holdsBow: rigBowPixels(player.team, aimStep, boardScale)
       else: rigMacePixels(player.team, aimStep, boardScale)),
      labelCogWeapon(color, bow = holdsBow),
      native = boardScale)
...
proc buildSpriteProtocolUpdates*(
  ...
  sim.addRotatingDiamonds(nextState.spriteDefs, currentIds, result)
  sim.addHorde(nextState.spriteDefs, currentIds, result, boardScale)
...
proc warmBoardRenderCaches*(sim: SimServer) =
  ...
  for team in sim.teams():
    for rot in 0 ..< SoldierRotations:
      ...
      discard rigMacePixels(team, rot, scale)
      discard rigBowPixels(team, rot, scale)
