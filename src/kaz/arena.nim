...
## (selectKazMap/loadKazMap — including the import-time default-arena
...
proc validateMapRect(name: string, rect: MapRect, width, height: int) =
  ...
  if rect.w <= 0 or rect.h <= 0:
    raise newException(KazError, "Map " & name & " size must be positive.")
  if rect.x < 0 or rect.y < 0 or
      rect.x + rect.w > width or rect.y + rect.h > height:
    raise newException(KazError, "Map " & name & " is outside the map.")
...
proc validateMap(gameMap: KazMap) =
  ## Raises if a loaded map has invalid geometry.
  if gameMap.width <= 0 or gameMap.height <= 0:
    raise newException(KazError, "Map dimensions must be positive.")
  case gameMap.layout
  of layoutSides:
    if gameMap.symmetry in {symRot90, symQuadMirror}:
      raise newException(
        KazError, "Sides maps cannot use a 4-team symmetry (rot90/quadmirror).")
  of layoutCorners, layoutPlus:
    if gameMap.symmetry notin {symRot90, symQuadMirror}:
      raise newException(
        KazError, "Corner/plus maps need rot90 or quadmirror symmetry.")
...
proc homeDepthOf(gameMap: KazMap): int =
...
proc rot90Quarter*(gameMap: KazMap, team: Team): int =
...
proc rot90TeamPoint*(gameMap: KazMap, red: MapPoint, team: Team): MapPoint =
...
proc teamImagePoint*(gameMap: KazMap, red: MapPoint, team: Team): MapPoint =
...
proc homeSlot*(gameMap: KazMap, team: Team): Team =
...
proc slotAnchor*(gameMap: KazMap, slot: Team): MapPoint =
...
proc teamAnchor*(gameMap: KazMap, team: Team): MapPoint =
...
proc spawnPocketHalf*(gameMap: KazMap, team: Team): tuple[w, h: int] =
...
proc plusArmHalf*(gameMap: KazMap): int =
...
proc plusArmBandOn*(gameMap: KazMap, span: int): tuple[lo, hi: int] =
...
proc defaultKazRooms(gameMap: KazMap): seq[Room] =
...
proc arenaKazMap(): KazMap =
  ...
  result.rooms = result.defaultKazRooms()
  result.validateMap()

proc arenaLargeKazMap(): KazMap =
...
proc captureZone*(gameMap: KazMap, team: Team): CaptureZone =
...
proc `==`*(a, b: ArenaShape): bool =
  ...
  ## KazMap values compare, which the map-spec round-trip tests rely on.
...
proc symmetryImages*(gameMap: KazMap, rect: MapRect): seq[MapRect] =
...
proc symmetryImages*(gameMap: KazMap, point: MapPoint): seq[MapPoint] =
...
proc buildArenaObstacles*(gameMap: KazMap): seq[ArenaShape] =
...
proc isSpinningDiamond*(gameMap: KazMap, shape: ArenaShape): bool {.inline.} =
...
proc buildAnimatedDiamonds*(
  gameMap: KazMap, obstacles: seq[ArenaShape]
...
proc centerPuddleSplat(rng: var MapRng, gameMap: KazMap): Puddle =
...
proc mapSizeScale(sizeName: string): float =
  ...
  of "colossal": 5.2  ## override-only (not in MapSizeNames): 2x giant.
    ...
    raise newException(KazError, "Unknown map size: " & sizeName)

proc scaledGenShell(sizeName: string): KazMap =
...
proc mapProtectedFloorAt*(gameMap: KazMap, x, y: int): bool =
...
proc validateMapWalkability*(gameMap: KazMap) =
...
proc scaledGenShell4(sizeName: string): KazMap =
...
proc sightlineLoX*(gameMap: KazMap): int =
...
proc sightlineHiX*(gameMap: KazMap): int =
...
proc sightlineLoY*(gameMap: KazMap): int =
...
proc sightlineHiY*(gameMap: KazMap): int =
...
proc rectOnOpenFloor(
  gameMap: KazMap, obstacles: seq[ArenaShape], rect: MapRect
): bool =
...
proc placePuddles(gameMap: var KazMap, count: int, rng: var MapRng) =
  ...
  for room in gameMap.defaultKazRooms():
    if room.name != "Center":
  ...
  proc addPuddlePair(
    gameMap: KazMap, splats: var seq[Puddle], splat: Puddle
  ): bool =
...
proc placePuddles*(gameMap: var KazMap, count: int, seed: int) =
  ...
  if count > MaxPuddles:
    raise newException(
      KazError, "Config field mapPuddles must be 0.." & $MaxPuddles & ".")
  if count > 0 and gameMap.symmetry in {symRot90, symQuadMirror}:
    raise newException(
      KazError, "Puddles are not supported on 4-team maps yet.")
  gameMap.puddles = @[]
...
proc generateMapAttempt*(
...
): KazMap =  ## One UNVALIDATED draw. Every top-level parameter is drawn unconditionally
  ...
  if teams == 4:
    ...
    if overrides.symmetry notin ["", "rot90", "quadmirror"]:
      raise newException(
        KazError, "4-team maps are rot90 or quadmirror; got mapSymmetry: " &
          overrides.symmetry)
    ...
    result.layout =
      ...
      of "plus": layoutPlus
        ...
        raise newException(
          KazError, "Unknown map layout: " & overrides.layout)
  else:
    ...
    result.symmetry =
      ...
      of "rot180": symRot180
        ...
        raise newException(
          KazError, "Unknown map symmetry: " & overrides.symmetry)
    if overrides.layout.len > 0 and overrides.layout != "sides":
      raise newException(
        KazError, "Map layout " & overrides.layout & " needs teams: 4.")
  ...
  block endzoneDraw:
    ...
    result.endzone =
      ...
      of "square": ezSquare
        ...
        raise newException(
          KazError, "Unknown map endzone: " & overrides.endzone)
    ...
    if teams == 4:
      raise newException(
        KazError, "Compact endzones (mapEndzone) need a 2-team map.")
    ...
    if result.homeDepth < HomeDepthMin or result.homeDepth > HomeDepthMax:
      raise newException(
        KazError, "Config field mapBaseDepth must be " & $HomeDepthMin &
          ".." & $HomeDepthMax & ".")
    if result.endzoneRadius < EndzoneRadiusMin or
        result.endzoneRadius > maxEndzoneRadius(result.width):
      raise newException(
        KazError, "Config field mapEndzoneRadius must be " &
  ...
  if feature notin CenterFeatureNames:
    raise newException(KazError, "Unknown map center feature: " & feature)
  ...
  if columns < 3 or columns > maxColumns:
    raise newException(
      KazError, "Config field mapColumns must be 3.." & $maxColumns & ".")
  ...
  if overrides.pits < -1 or overrides.pits > 64:
    raise newException(KazError, "Config field mapPits must be 0..64.")
  if overrides.puddles > MaxPuddles:
    raise newException(
      KazError, "Config field mapPuddles must be 0.." & $MaxPuddles & ".")
  if overrides.pitDensity < -1 or overrides.pitDensity > 1000:
    raise newException(
      KazError, "Config field mapPitDensity must be 0..1000.")
  let
  ...
  if result.symmetry in {symRot90, symQuadMirror}:
    ...
    if overrides.pits > 0:
      raise newException(
        KazError, "Trenches are not supported on 4-team maps yet.")
  ...
  block sightlineRepair:
    proc rowBlocked(gameMap: KazMap, y: int): bool =
      for x in gameMap.sightlineLoX .. gameMap.center.x:
        if mapWallAt(gameMap, gameMap.leftObstacles, x, y):
          return true
      false
    proc rowBlockedFull(gameMap: KazMap, obstacles: seq[ArenaShape],
        y: int): bool =
  ...
  block columnSightlineRepair:
    ...
    proc colBlocked(gameMap: KazMap, x: int): bool =
      ...
      for y in gameMap.sightlineLoY .. gameMap.sightlineHiY:
        if mapWallAt(gameMap, gameMap.leftObstacles, x, y,
          ...
          return true
      false
    proc colFullyProtected(gameMap: KazMap, x: int): bool =
  ...
  if windowCount > 6:
    raise newException(KazError, "Config field mapWindows must be 0..6.")
  ...
  block finalizeTrenches:
    ...
    if oddCenterPit:
      ...
      digs.add centerPit
    proc addPair(
      gameMap: KazMap, digs: var seq[MapRect], trench: MapRect
    ): bool =
...
proc validateGeneratedMap*(gameMap: KazMap): string =
...
proc shapeFromSpecNode(node: JsonNode): ArenaShape =
  ...
  of "polygon":
    ...
    raise newException(
      KazError, "Unknown map spec shape: " & node["kind"].getStr())
...
proc mapSpecJson*(gameMap: KazMap): string =
...
proc mapFromSpecJson*(text: string): KazMap =
  ...
  try:
    ...
    raise newException(KazError, "Could not parse map spec JSON: " & e.msg)
  ...
  result.symmetry =
    ...
    of "none": symNone          # coworld-ctf#280 full-board authoring (no lift)
      ...
      raise newException(
        KazError, "Unknown map spec symmetry: " & symmetryText)
  ...
  result.layout =
    ...
    of "plus": layoutPlus
      ...
      raise newException(KazError, "Unknown map spec layout: " & layoutText)
  ...
  result.endzone =
    ...
    of "square": ezSquare
      ...
      raise newException(KazError, "Unknown map spec endzone: " & endzoneText)
...
proc resolveKazMapMetadata*(config: GameConfig): KazMap =
  ...
  result =
    if config.mapSpec.len > 0:
      ...
      case name
      of ArenaName: arenaKazMap()
      of ArenaLargeName: arenaLargeKazMap()
      of GenMapName: generateKazMap(genSeed, config.mapGen, config.teams)
      else:
        raise newException(KazError, "Unknown map: " & name)
  if result.teamCount() != config.teams:
    raise newException(
      KazError, "Config asks for " & $config.teams & " teams but map " &
...
## The SELECTED map's layout, installed once per process by loadKazMap and
...
proc selectKazMap(gameMap: KazMap) =
...
proc installDefaultArena*() =
  ...
  ## import-time `selectKazMap(arenaKazMap())` side effect with this explicit
  ## call: constructing any sim (initSimServer -> loadKazMap) installs its
  ...
  selectKazMap(arenaKazMap())

proc loadKazMapMetadata*(path = ""): KazMap =
  ...
  of ArenaLargeName: arenaLargeKazMap()
    ...
    if parts.len <= 2 and parts[0] == GenMapName:
      generateKazMap(suffix)
    else:
      raise newException(KazError, "Unknown map: " & name)

proc loadKazMapMetadata*(config: GameConfig): KazMap =
  ## GameConfig-driven metadata: honors mapSpec, mapSeed, pool picks, and
  ## the generator overrides.
  resolveKazMapMetadata(config)

proc loadKazMap*(path = ""): KazMap =
  ...
  result = loadKazMapMetadata(path)
  selectKazMap(result)

proc loadKazMap*(config: GameConfig): KazMap =
  ## Resolves the config's effective map and installs it as this process's
  ## arena.
  result = resolveKazMapMetadata(config)
  selectKazMap(result)
...
proc arenaCenterOffset2(x, y, cx, cy: int): tuple[dx, dy: int] {.inline.} =
  ## The installed-map twin of KazMap.centerOffset2: twice the offset from
...
proc mapProtectedFloorAtF*(
  gameMap: KazMap, x, y: float, cx, cy: int
): bool =
