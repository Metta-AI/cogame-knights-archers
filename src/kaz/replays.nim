...
const
  ...
  KazReplayMagic = "COWLDKAZ"
  KazReplayFormatVersion = 1'u16
  KazReplaySpec = ReplaySpec(
    magic: KazReplayMagic,
    formatVersion: KazReplayFormatVersion,
    gameName: GameName,
...
proc advanceReplayGame(sim: var SimServer) =
  ## The playback mirror of the server's WAVE SWITCH (its named edit #3).
  ## `gameIndex` and the `waveLog` arrays are written only by the live loop and
  ## none of them is in `gameHash` — so without this the readouts a spectator
  ## reads off a replay ("WAVE 1/2 · 17 alive") stay frozen on wave one for the
  ## whole episode and the second wave is never announced. Archiving here is
  ## also what lets the momentum series stay cumulative across both waves.
  if not sim.hordeLoadout():
    return
  sim.archiveWave()
  sim.gameIndex = sim.waveTicksLog.len
...
proc scanTeamLead(sim: SimServer): seq[int] =
  ...
  ##
  ## Horde: cumulative TEAM KILLS and the leader's PRESSURE PERCENT.
  if sim.hordeLoadout():
    ## TWO SERIES over the whole EPISODE, not one per team: the squad's
    ## cumulative kills and the leader's pressure percentage. There is only one
...
proc advanceReplayScan*(replay: var ReplayPlayer, maxTicks: int) =
  ...
  while stepsLeft > 0 and scan.builder.playing and
    ...
    for event in stepBeats:
      ## The five kinds the appended viewer block draws markers for, and no
      ## others: it has CSS for exactly these and calls its OWN builder, so an
      ## unlabelled div marker can never appear. Kills stay out — dozens of
      ## same-looking ticks would bury the beats that matter.
      let scrubberBeats =
        if scan.sim.hordeLoadout():
