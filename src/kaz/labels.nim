...
const
  ...
  LabelZombie* = "zombie"
    ## One of the walking dead on the board. The horde shares one label across
    ## every rotation and shamble frame: a spectator (and a bot reading the
    ## board stream) needs "this is a zombie", not which frame it is on.
  LabelArrow* = "arrow"
    ## One archer's arrow in flight.
  ...
  LabelPrefixCogMace* = "cog mace "
    ## A knight's held mace, `cog mace <color>` (board stream only).
  LabelPrefixCogBow* = "cog bow "
    ## An archer's held bow, `cog bow <color>`, which REPLACES the mace
    ## sprite on an archer seat — so the silhouette shows the role.
...
proc labelCogWeapon*(color: string; bow: bool): string =
  ## The held-weapon sprite label on the board rig: `cog bow <color>` for an
  ## archer, `cog mace <color>` for a knight. A hero holds exactly one thing,
  ## decided by its role, so these two are mutually exclusive per hero.
  (if bow: LabelPrefixCogBow else: LabelPrefixCogMace) & color
