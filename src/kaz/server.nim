...
proc httpHandler(request: Request) =
  if request.path == HealthPath and request.httpMethod == "GET":
    ...
    # STATIC wasm bundle is what the platform serves for a hosted replay; this
    # route exists for local inspection and for the certifier's HTTP contract
    # probe. (ELEVATE-BY-REBUILD: our HTML, not bitworld's.)
    ...
    request.respond(200, headers, "knights-archers server")
...
proc runServerLoop*(
  ...
  ## The horde episode: `num_agents` seats each drive exactly ONE hero. The
  ## seats join as the starter's players do (slots 0..num_agents-1,
  ## token-checked); from then on every actuator mask comes from the control
  ## layer rather than from a socket.
  ...
  while true:
    ...
    if squadMode and not deadlineHit and
      ...
      echo "wall-clock budget of ", config.wallClockBudgetSeconds,
        "s reached; settling the episode from the kills banked at this tick"
      ## The wave in progress banks its kills with NO clear bonus; the waves
      ## already finished keep their value. The replay is complete up to the
      ## stop tick and the game-over frame is written.
    ...
    {.gcsafe.}:
      withLock appState.lock:
        ...
        if squadMode and not squadsBuilt and sim.lobbyJoinTimedOut():
          ...
          # no-show to the platform (lowest missing slot only), then start
          # anyway: that seat's hero runs the published `phalanx` baseline for
          ...
          declarePlayerFailure(
            ...
            "player slot " & $stuckSlot & " never joined the lobby within " &
              ...
              "s); its hero plays the phalanx baseline"
          )
            ...
            if squadMode:
              ...
              if firstRegistration:
                replayWriter.writeChat(
                  ...
                  registerRecord(
                    playerIndex,
                    sim.cogAlias(playerIndex),
                    sim.roleForSeat(playerIndex),
                    policy.label,
                    engine.policyKind(playerIndex),
                    $policy.baseline
                  )
      ...
      if sim.gameTicksElapsed() mod turnTicks == 0 and turnKey != lastTurnKey:
        ...
        for seat in 0 ..< engine.directives.len:
          ...
          let record = directive.boundedDirectiveRecord(
            ...
            sim.cogAlias(seat), sim.roleForSeat(seat))
        ...
        if sim.seatCommands(seat, cogIndex) and seat < engine.directives.len and
          ...
          for candidate in engine.directives[seat].orders:
            if candidate.cogIndex == cogIndex:
              ...
              break
        if not found:
          # This hero's seat has no directive yet (turn 0, or a seat that has
          # not registered). It plays the published `phalanx` baseline, which
          # is what "adapt to a partner you know the rules of" means here.
          let scripted = engine.phalanxFor(sim, @[cogIndex])
    ...
    if replayLoaded:
      ...
      for _ in 0 ..< playbackSpeed(liveSpeedIndex):
        ...
        try:
          ...
          if not squadMode:
            raise
          echo "knights-archers: SIM GUARD tripped at tick ", sim.tickCount, ": ",
            guard.msg
          faultRule = EndRuleSimFault
        except CatchableError as error:
          if not squadMode:
            raise
          echo "knights-archers: HOST ERROR at tick ", sim.tickCount, ": ",
            error.msg
        ...
        if config.maxGames > 0 and phaseBeforeStep != GameOver and
          ...
          inc gamesPlayed
          if squadMode:
            # Archive the wave that just ended BEFORE the lobby reset that
            # precedes the next startGame: resetToLobby clears the horde and
            # every per-wave counter, so the numbers have to be banked here.
            sim.archiveWave()
            sim.gameIndex = gamesPlayed
            squadsBuilt = false
            lastTurnKey = -1
            echo "wave ", gamesPlayed, " done: ", sim.waveEndRuleLog[^1],
              ", kills=", sim.waveKillsLog[^1],
              ", closest call=", sim.waveClosestLog[^1], "px"
