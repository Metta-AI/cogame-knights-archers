...
proc randomSeed(): int =
  ...
  if not urandom(buf):
    raise newException(KazError, "OS entropy source unavailable")
...
proc echoStartupConfig(
  ...
  echo "knights-archers config: host=", runtimeConfig.host,
...
when isMainModule:
  let
    ...
    localReplayPath =
      if runtimeConfig.replayUri.len > 0:
        getTempDir() / ("knights-archers-replay-" & $getCurrentProcessId() &
          ".bitreplay")
      else:
        ""
  ...
  let loadReplayPath =
    if runtimeConfig.replayMode:
      let path = getTempDir() / ("knights-archers-load-replay-" &
  ...
  echo "starting knights-archers on ", runtimeConfig.host, ":", runtimeConfig.port
  runServerLoop(
    runtimeConfig.host,
    runtimeConfig.port,
    config,
